...
trans1_raw <-read.csv("c:\\users\\david\\OneDrive - University of Texas at Arlington\\latency\\trans1.csv", header=TRUE)
trans2_raw <-read.csv("c:\\users\\david\\OneDrive - University of Texas at Arlington\\latency\\trans2.csv", header=TRUE)
...
trans1$minute<-floor((trans1$trans_begin)/60)
...
trans1$ltnc_cat3<-factor(trans1$ltnc_cat3,levels=c('low','high'))
trans1$ltnc_cat2<-factor(trans1$ltnc_cat2,levels=c('Low latency','Med. latency','High latency'))
trans1$graph_block<-factor(trans1$graph_block,levels=c('init','wrmp','base','ltnc','post'))
trans1$block<-factor(trans1$graph_block,levels=c('base','wrmp','ltnc','post','init'))
trans1$minute_f <- factor(trans1$minute,levels=c('0','1',
'2',
'3',
'4',
'5',
...
trans1.woinit<-sqldf("
 select *
  from trans1
  where min_block != 'init'
")
trans1.woinit$min_block<-factor(trans1.woinit$min_block,levels=c('wrmp','base','ltnc','post'))
...
")
abcd1$ltnc_cat2<-factor(abcd1$ltnc_cat2,levels=c('Low latency','Med. latency','High latency'))
abcd1$graph_block<-factor(abcd1$graph_block,levels=c('wrmp','base','ltnc','post'))
abcd1$block<-factor(abcd1$graph_block,levels=c('base','wrmp','ltnc','post','init'))
...
base_ltnc1 <- sqldf("
 select a.conv, b.ltnc_ms, c.ltnc_cat, c.ltnc_cat2,
    ...
    b.mean_gap - a.mean_gap mean_gap_diff
   from abcd1 a
    join abcd1 b on a.conv = b.conv and a.graph_block = 'base' 
    join conv c on c.conv = a.conv
   where b.graph_block = 'ltnc'
")
...
base_post1 <- sqldf("
 select a.conv, c.ltnc_ms, c.ltnc_cat, c.ltnc_cat2,
   ...
   where b.graph_block = 'post'
")
base_post1$ltnc_cat2<-factor(base_post1$ltnc_cat2,levels=c('Low latency','Med. latency','High latency'))
...
trans2$minute<-floor((trans2$trans_begin)/60)
...
trans2$ltnc_cat3<-factor(trans2$ltnc_cat3,levels=c('low','high'))
trans2$ltnc_cat2<-factor(trans2$ltnc_cat2,levels=c('Low latency','Med. latency','High latency'))
trans2$graph_block<-factor(trans2$graph_block,levels=c('init','wrmp','base','ltnc','post'))
trans2$block<-factor(trans2$graph_block,levels=c('base','wrmp','ltnc','post','init'))
trans2$minute_f <- factor(trans2$minute,levels=c('0','1',
'2',
'3',
'4',
'5',
...
trans2.woinit<-sqldf("
 select *
  ...
  where min_block != 'init'
")
trans2.woinit$min_block<-factor(trans2.woinit$min_block,levels=c('wrmp','base','ltnc','post'))
...
")
abcd2$ltnc_cat2<-factor(abcd2$ltnc_cat2,levels=c('Low latency','Med. latency','High latency'))
abcd2$graph_block<-factor(abcd2$graph_block,levels=c('wrmp','base','ltnc','post'))
abcd2$block<-factor(abcd2$graph_block,levels=c('base','wrmp','ltnc','post','init'))
...
base_ltnc2 <- sqldf("
 select a.conv, b.ltnc_ms, c.ltnc_cat, c.ltnc_cat2,
    ...
    b.mean_gap - a.mean_gap mean_gap_diff
   from abcd2 a
    join abcd2 b on a.conv = b.conv and a.graph_block = 'base' 
    join conv c on c.conv = a.conv
   where b.graph_block = 'ltnc'
")
...
base_post2 <- sqldf("
 select a.conv, c.ltnc_ms, c.ltnc_cat, c.ltnc_cat2,
   ...
   where b.graph_block = 'post'
")
base_post2$ltnc_cat2<-factor(base_post2$ltnc_cat2,levels=c('Low latency','Med. latency','High latency'))

#marginal differences between the end of the latency period and the beginning of post
#s/b ok to just base_post
sqldf("select sum(mean_gap_diff), sum(otr_diff), sum(trans_rate_diff), sum(overlap_spm_diff), sum(gap_spm_diff) from base_post1")
sqldf("select sum(mean_gap_diff), sum(otr_diff), sum(trans_rate_diff), sum(overlap_spm_diff), sum(gap_spm_diff) from base_post2")

sqldf("select * from base_post except select * from base_post2")
sqldf("select * from base_post except select * from base_post1")

#weighted
abcd.wt<-sqldf("
 select *, 
   case when graph_block = 'ltnc' then 1 else 0 end perspective,
   case when graph_block = 'ltnc' then 0.5 else 1 end wt,
   case when graph_block = 'ltnc' then 'ltnc1' else graph_block end graph_block_persp,
   max(ltnc_ms) over (partition by conv) ltnc_ms_const
  from abcd1
  union
 select *, 2 perspective, 0.5 wt,'ltnc2' graph_block_persp, ltnc_ms ltnc_ms_const
  from abcd2
  where graph_block = 'ltnc'
")
abcd.wt$conv <- as.factor(abcd.wt$conv)
abcd.wt$perspective <- as.factor(abcd.wt$perspective)
abcd.wt$graph_block_persp <- factor(abcd.wt$graph_block_persp,levels=c('wrmp','base','ltnc1','ltnc2','post'))
abcd.wt$graph_block<-factor(abcd.wt$graph_block,levels=c('init','wrmp','base','ltnc','post'))
abcd.wt$block<-factor(abcd.wt$graph_block,levels=c('base','wrmp','ltnc','post','init'))

trans.wt<-sqldf("
 select *, case when graph_block = 'ltnc' then graph_block || perspective else graph_block end graph_block_perspective,
  max(ltnc_ms) over (partition by conv) ltnc_ms_const,
  case when graph_block = 'ltnc' or graph_block = 'post' then max(ltnc_ms) over (partition by conv) else 0 end ltnc_ms_lp
 from
 (
 select *, 
   ...
   case when graph_block = 'ltnc' then 0.5 else 1 end wt
  from trans1
   where graph_block != 'init'
  union
 select *, 2 perspective, 0.5 wt
  from trans2
  where graph_block = 'ltnc'
 )
")
trans.wt$graph_block<-factor(trans.wt$graph_block,levels=c('init','wrmp','base','ltnc','post'))
trans.wt$block<-factor(trans.wt$graph_block,levels=c('base','wrmp','ltnc','post','init'))
trans.wt$graph_block_perspective<-factor(trans.wt$graph_block_perspective,levels=c('init','wrmp','base','ltnc1','ltnc2','post'))
trans.wt$perspective<-as.factor(trans.wt$perspective)

trans.wt.winit<-sqldf("
 select *, case when graph_block = 'ltnc' then graph_block || perspective else graph_block end graph_block_perspective,
  max(ltnc_ms) over (partition by conv) ltnc_ms_const,
  case when graph_block = 'ltnc' or graph_block = 'post' then max(ltnc_ms) over (partition by conv) else 0 end ltnc_ms_lp
 from
...
")
trans.wt.winit$graph_block<-factor(trans.wt.winit$graph_block,levels=c('init','wrmp','base','ltnc','post'))
trans.wt.winit$block<-factor(trans.wt.winit$graph_block,levels=c('base','wrmp','ltnc','post','init'))
trans.wt.winit$graph_block_perspective<-factor(trans.wt.winit$graph_block_perspective,levels=c('init','wrmp','base','ltnc1','ltnc2','post'))
trans.wt.winit$perspective<-as.factor(trans.wt.winit$perspective)
